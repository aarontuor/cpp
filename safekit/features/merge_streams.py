...
class Merge:
    ...
    >>>data = merge_streams.Merge(file_list=['short_t_toy_auth.txt', 'short_t_toy_proc.txt'],
                                  sort_column='time', delimiter=',')
    >>>for event_type, event in data():
    ...    print('event_type: %s event: %s' % (event_type, event))
    event_type:short_t_toy_auth.txt event:['1', 'U7@DOM1', 'U6@DOM1', 'C606', 'C606', '?', '?', 'AuthMap', 'Success']
    ....
    event_type:short_t_toy_auth.txt event:['3', 'U7@DOM1', 'U6@DOM1', 'C1183', 'C1183', '?', '?', 'AuthMap', 'Success']
    """
